...
namespace Sichem
{
	internal class ConversionProcessor : BaseProcessor
	{
		...
		protected override TextWriter Writer
		{
			get
			{
				...
				if (!_writers.TryGetValue(_currentSource, out sw))
				{
					...
					sw.Write("unsafe {0} class {1}\n\t{{\n",
						_parameters.IsPartial ? "partial" : string.Empty,
						_parameters.Class);
		...
		private string SetSource(string name, Func<string, string> sourceGetter)
		{
			...
			switch (curKind)
			{
				...
				case CXCursorKind.CXCursor_StructDecl:
					...
					if (!_visitedStructs.Contains(structName) &&
						...
						!string.IsNullOrEmpty(SetSource(structName, _parameters.StructSource)))
					{
						Logger.Info("Processing struct {0}", structName);

						_isStruct = Parameters.TreatStructAsClass == null || !Parameters.TreatStructAsClass(structName);
		...
		private CXChildVisitResult VisitEnums(CXCursor cursor, CXCursor parent, IntPtr data)
		{
			...
			if (curKind == CXCursorKind.CXCursor_EnumDecl)
			{
				...
				if (string.IsNullOrEmpty(SetSource(n, Parameters.EnumSource)))
				{
					return CXChildVisitResult.CXChildVisit_Continue;
				}
		...
		private CXChildVisitResult VisitFunctions(CXCursor cursor, CXCursor parent, IntPtr data)
		{
			...
			if (curKind == CXCursorKind.CXCursor_FunctionDecl)
			{
				...
				if (string.IsNullOrEmpty(SetSource(_functionName, Parameters.FunctionSource)))
				{
					return CXChildVisitResult.CXChildVisit_Continue;
				}
			...
			for (uint i = 0; i < numArgTypes; ++i)
			{
				...
				if (typeName.EndsWith("*"))
				{
					if (Parameters.UseRefInsteadOfPointer != null &&
					    ...
					    (Parameters.TreatStructAsClass == null || !Parameters.TreatStructAsClass(typeName)) &&
